...
describe('GET Products by brand', () =>{
    ...
    test('Responds with validation errors, status 400: Invalid reference (brand name).',
        async() =>{
    ...
    test('Responds with an empty array, status 200: No documents match requested '+
        'creteria (manufacturer name).',
        async() =>{
            const response = await request(app).get(
    ...
    test('Responds with paginated resource, status 200: Default pagination => 10.', 
        async() =>{
            const response = await request(app).get(
